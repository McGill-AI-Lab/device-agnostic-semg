...
For example, `config/model/LUNA_base.yaml` defines the default parameters for the base `LUNA` model.
...
For example, the `LUNA_finetune.yaml` experiment file specifies that for this experiment, we should use the `finetune_data_module`, a `LUNA` model size (base/large/huge), the `cosine` scheduler, and so on.
...
### Basic Command
This command runs the `LUNA_pretrain` experiment using the settings defined in `config/experiment/LUNA_pretrain.yaml`:
```bash
python run_train.py +experiment=LUNA_pretrain
```
...
```bash
python run_train.py +experiment=LUNA_pretrain optimizer.lr=1e-5 batch_size=128
```
This command will use the `LUNA_pretrain` experiment configuration but override the learning rate to `1e-5` and batch size to `128`, overriding the values in the YAML files. This is perfect for quick experiments and hyperparameter tuning without needing to create new config files.
